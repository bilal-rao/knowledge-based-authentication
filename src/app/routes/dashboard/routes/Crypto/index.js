...
import { showModuleLoader, fetchModule } from 'actions/Module';
...
componentWillReceiveProps(props) {
  ...
  logUser(user) {
    ...
    if (env === 'production') {
         LogRocket.identify('ixdgzi/offerings-uat', {
             name: user.name,
  ...
  render() {
    return (
      <div className="dashboard animated slideInUpTiny animation-duration-3">
        ...
        <ContainerHeader match={this.props.match} title={<IntlMessages id="sidebar.dashboard.crypto" />} />
...
function mapStateToProps(state) {
  return {
      user: state.modules.moduleData ? state.modules.moduleData.data : ""
  }
}
