...
// Actions
import {
  showUserMainPageLoader,
  fetchUser,
  hideUserMessage,
  hideUserMainPageMessage,
...
class DataTable extends React.Component {
  ...
  componentDidUpdate() {
    if (this.props.deleteUserSuccess) {
    ...
    if (this.props.showMessage) {
      ...
      setTimeout(() => {
        this.props.hideUserMainPageMessage();
      }, 100);
    }
  }
  ...
  onHandleClick = (mode, id) => {
    ...
    this.props.history.push(
      `/app/identity-management/users/approved/${mode}/${id}`
    );
  };
  ...
  render() {
    ...
    return (
      <Paper>
        <Toolbar>
          <div className="title">
            <Typography type="title">Users Listing</Typography>
          </div>
          <div className="actions">
            ...
            <IconButton
              ...
              onClick={() =>
                this.props.history.push(
                  "/app/identity-management/users/approved/add"
                )
              }
            >
        ...
        <div className="flex-auto">
          <div className="table-responsive-material">
            {this.props.loader === false && this.props.usersList ? (
              <Table className="">
                <DataTableHead
                        ...
                        <TableCell>
                          {
                            n.status === 0 ? (
                              ...
                              <Badge color="warning" className="text-white">Force Password Change</Badge>
                            ) : (
                              <Badge color="light">Blocked</Badge>
                            )
                          }
                        </TableCell>
                        <TableCell>
                          <div style={{ display: "inline-flex" }}>
                            <IconButton
                              ...
                              aria-label="View"
                              onClick={() => {
                                this.onHandleClick("view", n.userId);
                              }}
                            >
                              <Tooltip title="View">
                                <ViewIcon />
                            ...
                            <IconButton
                              ...
                              aria-label="Edit"
                              onClick={() => {
                                this.onHandleClick("edit", n.userId);
                              }}
                            >
                              <Tooltip title="Edit">
                                <EditIcon />
...
function mapStateToProps(state) {
  return {
    ...
    usersList: state.usersData.usersList
      ? state.usersData.usersList.data.items
      : "",
    deleteUserSuccess: state.usersData.deleteUserSuccess,
    paging: state.usersData.usersList
      ? state.usersData.usersList.data.paging
      : "",
    showMessage: state.usersData.showMessage,
    alertMessage: state.usersData.alertMessage,
    loader: state.usersData.loader,
    showMainPageMessage: state.usersData.showMainPageMessage,
    alertMainPageMessage: state.usersData.alertMainPageMessage,
